...
if args.dataset_name == 'ml-10m':
    with open(f'../data/{dataset_name}.pkl', 'rb') as f:
        ...
        U = np.array(pickle.load(f, encoding='latin1')) 
        Vh = np.array(pickle.load(f, encoding='latin1')) 
        initial_embedding = [U[:, :embed_dim], Vh[:, :embed_dim]]
    ...
    for epo in range(epochs):
        for i, (fields, target) in enumerate(tk0):
            ...
            if now_iter % test_iter == 0:
                ...
                print(f"validation auc: {auc}, loss: {log_losses}")
...
for now_run in range(args.runs):
    ...
    if model_name == 'macgnn':
        model = MacGNN(field_dims=field_dims, u_group_num=u_cluster_num, i_group_num=i_cluster_num,
                       embed_dim=embed_dim, recent_len=recent_len, initial_embedding=initial_embedding, tau=args.tau, device=device).to(device)
    else:
        raise Exception("no model selected!")

    criterion = torch.nn.BCELoss()
    
    optimizer = torch.optim.NAdam(params=model.parameters(), lr=learning_rate, weight_decay=weight_decay)
